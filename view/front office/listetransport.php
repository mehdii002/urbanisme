...
if (isset($_GET['categorie']) && $_GET['categorie'] !== "") {
    ...
    $listTransports = $transportC->getTransportsWithCategorie();
}
?>
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-xl-9 col-lg-8 content-left-offset">
                    ...
                    <h3 class="page-title">Liste des entrepreneurs</h3>
                    ...
                    <div class="freelancers-container freelancers-list-layout compact-list margin-top-35">
                        ...
                        <!--Freelancer -->
                        <?php
foreach($listTransports as $transportData) {
    // Créez un objet Transport à partir des données du tableau
    $transport = new Transport(
        $transportData['id_transport'],
        $transportData['nom_transport'],
        $transportData['capacite'],
        $transportData['frequence'],
        $transportData['zone_deservie'],
        $transportData['cout_moyen'],
        $transportData['ecologique'],
        $transportData['id_categorie'],            // ✅ not nom_categorie! must be id_categorie
        $transportData['start_position'],          // ✅ start_position
        $transportData['destination_position']     // ✅ destination_position
    );
?>
                        <!-- Transport -->
                        <a href="single-transport-page.php?id=<?php echo $transportData['id_transport']; ?>"
                            class="task-listing">
                            <!-- Transport Listing Details -->
                            <div class="task-listing-details">
                                <!-- Details -->
                                <div class="task-listing-description">
                                    <h3 class="task-listing-title">
                                        <?php echo htmlspecialchars($transportData['nom_transport']); ?></h3>
                                    <ul class="task-icons">
                                        <li><i class="icon-material-outline-directions-transit"></i> Catégorie:
                                            <?php echo htmlspecialchars($transportData['nom_categorie']); ?></li>
                                        <li><i class="icon-material-outline-people"></i> Capacité:
                                            <?php echo htmlspecialchars($transportData['capacite']); ?> personnes</li>
                                        <li><i class="icon-material-outline-access-time"></i> Fréquence:
                                            <?php echo htmlspecialchars($transportData['frequence']); ?> min</li>
                                        <li><i class="icon-material-outline-location-on"></i> Zone:
                                            <?php echo htmlspecialchars($transportData['zone_deservie']); ?></li>
                                    </ul>
                                </div>
                            </div>
                            ...
                            <div class="task-listing-bid">
                                <div class="task-listing-bid-inner">
                                    <div class="task-offers">
                                        <strong><?php echo htmlspecialchars($transportData['cout_moyen']); ?> €</strong>
                                        <span>Coût moyen</span>
                                    </div>
                                    <div class="task-offers">
                                        <strong><?php echo $transportData['ecologique'] ? 'Eco' : 'Non-eco'; ?></strong>
                                        <span>Type</span>
                                    </div>
                                    <span class="button button-sliding-icon ripple-effect">Choisir <i
                                            class="icon-material-outline-arrow-right-alt"></i></span>
                                </div>

                                <!-- 🚀 Google Maps Button -->
                                <?php if (!empty($transportData['start_position']) && !empty($transportData['destination_position'])): ?>
                                <div style="margin-top: 10px;">
                                    <a href="https://www.google.com/maps/dir/?api=1&origin=<?= urlencode($transportData['start_position']) ?>&destination=<?= urlencode($transportData['destination_position']) ?>"
                                        target="_blank" class="button gray ripple-effect">
                                        Itinéraire <i class="icon-material-outline-map"></i>
                                    </a>
                                </div>
                                <?php endif; ?>
                                <!-- End of Google Maps Button -->
